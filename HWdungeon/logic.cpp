 ...
 */
char** createMap(const int maxRow, const int maxCol) {
    // char** theMap = new char[maxRow][maxCol] { TILE_OPEN };
    char** theMap = new char*[maxRow]{};
    for(int i = 0; i < maxRow; i++)
    {
        theMap[i] = new char[maxCol]{};
        for(int k = 0; k < maxCol; k++)
        {
            theMap[i][k] = TILE_OPEN;
        }
    }
...
bool doMonsterAttack(char** map, int maxRow, int maxCol, Player& player) {
    ...
    // check up
    for(int u = player.row-1; u >= 0; u--)
    {
        if(map[u][player.col] == TILE_MONSTER)
        {
            map[u][player.col] = TILE_OPEN;
            map[u+1][player.col] = TILE_MONSTER;
        }
        else if(map[u][player.col] == TILE_PILLAR)
        {
            break;
        }
    }
    // check down
    for(int d = player.row+1; d < maxRow; d++)
    {
        if(map[d][player.col] == TILE_MONSTER)
        {
            map[d][player.col] = TILE_OPEN;
            map[d-1][player.col] = TILE_MONSTER;
        }
        else if(map[d][player.col] == TILE_PILLAR)
        {
            break;
        }
    }
    // check left
    for(int l = player.col-1; l >= 0; l--)
    {
        if(map[player.row][l] == TILE_MONSTER)
        {
            map[player.row][l] = TILE_OPEN;
            map[player.row][l+1] = TILE_MONSTER;
        }
        else if(map[player.row][l] == TILE_PILLAR)
        {
            break;
        }
    }
    // check right
    for(int r = player.col+1; r < maxCol; r++)
    {
        ...
        else if(map[player.row][r] == TILE_PILLAR)
        {
            break;
        }
    }
