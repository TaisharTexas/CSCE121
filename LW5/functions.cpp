...
bool loadImage(const char filename[], Pixel image[MAX_WIDTH][MAX_HEIGHT], int width, int height) {
  ...
  for(int row = 0; row < height; row++)
  {
      for(int col = 0; col < width; col++)
      {
          ...
          ifs >> lineValueR >> lineValueG >> lineValueB;
...
void grayscaleImage(Pixel image[MAX_WIDTH][MAX_HEIGHT], int width, int height) {
    ...
    for(int col = 0; col < width; col++)
    {
        for(int row = 0; row < height; row++)
        {
            int average = round((image[col][row].r + image[col][row].g + image[col][row].b) / 3.0);
            image[col][row].r = round(average);
            image[col][row].g = round(average);
            image[col][row].b = round(average);
        }
    }
}
